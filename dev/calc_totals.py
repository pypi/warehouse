...
def main():
    ...
    release_query = sa.select([Release.project_id,
                               func.array_agg(Release.id)])\
        .group_by(Release.project_id)
    for release in connection.execute(release_query):
        files_query = sa.select([func.sum(File.size)])\
            .where(File.release_id.in_(release.array_agg_1))
        for files in connection.execute(files_query):
            connection.execute(
                sa.update(Project).values(total_size=files.sum_1)
