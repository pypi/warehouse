...
evaluate in the *parent* template's context. The rendered component sees only the
isolated context returned by its ``get_context`` (see registry.render). Per-render
variables that pyramid_jinja2 injects into page templates (``request``, ``context``) are
not available inside component templates; evaluate request-dependent expressions in the
parent and pass the results in as props. Reaching for one anyway raises
``UndefinedError`` (see ``registry._strict_environment``).
"""
