...
class TestGitHubAssociationConnect:
    def test_initiates_oauth_flow(self, pyramid_request, oauth_provider_service):
        ...
        assert (
            result.location
            == "http://localhost?code=mock_github_authorization_code&state="
...
class TestGitLabAssociationConnect:
    def test_initiates_oauth_flow(self, pyramid_request, mocker):
        """Test that the GitLab connect view generates state and redirects."""
        # Create a mock GitLab OAuth service
        mock_gitlab_service = mocker.Mock()
        mock_gitlab_service.generate_authorize_url.return_value = (
            "https://gitlab.com/oauth/authorize?state=test"
        )

        # Mock find_service to return our mock for gitlab
        pyramid_request.find_service = lambda iface, name="": mock_gitlab_service

        result = views.gitlab_association_connect(pyramid_request)

        assert isinstance(result, HTTPSeeOther)
        assert result.location == "https://gitlab.com/oauth/authorize?state=test"
        assert "gitlab_oauth_state" in pyramid_request.session
        mock_gitlab_service.generate_authorize_url.assert_called_once_with(
            pyramid_request.session["gitlab_oauth_state"]
        )
...
class TestGitLabAssociationCallback:
    ...
    def test_oauth_error_from_provider(self, pyramid_request):
        """Test handling of OAuth error from GitLab."""
        session_state = "valid_state"
    ...
    def test_missing_authorization_code(self, pyramid_request):
        """Test handling when no code is received."""
        session_state = "valid_state"
        pyramid_request.session["gitlab_oauth_state"] = session_state
        pyramid_request.GET = MultiDict({"state": session_state})
        pyramid_request.route_path = lambda *args: "/manage/account/"
    ...
    def test_no_access_token_received(self, pyramid_request, mocker):
        """Test handling when token exchange returns no access token."""
        session_state = "valid_state"
        pyramid_request.session["gitlab_oauth_state"] = session_state
        pyramid_request.GET = MultiDict({"state": session_state, "code": "auth_code"})
        ...
        # Create a mock GitLab OAuth service
        mock_gitlab_service = mocker.Mock()
        mock_gitlab_service.exchange_code_for_token.return_value = {}

        original_find_service = pyramid_request.find_service

        def mock_find_service(iface, name=""):
            if name == "gitlab":
                return mock_gitlab_service
            return original_find_service(iface, name=name)

        pyramid_request.find_service = mock_find_service
    ...
    def test_successful_association(
        ...
        mock_gitlab_service.get_user_info.return_value = {
            "id": 99999,
            "username": "gitlabuser",
        }
        ...
        assert result.location == "/manage/account/"
        # Verify association was created
        assert pyramid_user.account_associations[0].service == "gitlab"
        assert pyramid_user.account_associations[0].external_username == "gitlabuser"
        # Verify event was recorded
        pyramid_user.record_event.assert_called_once()
        assert (
            pyramid_user.record_event.call_args[1]["tag"]
    ...
    def test_generic_exception_during_oauth(self, pyramid_request, mocker):
        """Test handling of unexpected exceptions during OAuth flow."""
        session_state = "valid_state"
        pyramid_request.session["gitlab_oauth_state"] = session_state
        pyramid_request.GET = MultiDict({"state": session_state, "code": "auth_code"})
        ...
        # Create a mock GitLab OAuth service that raises an exception
        mock_gitlab_service = mocker.Mock()
        mock_gitlab_service.exchange_code_for_token.side_effect = Exception(
            "Network error"
        )
