...
class TestManageAccountConnectedAccounts:
    def test_manage_account_connected_accounts(self, db_request, mocker):
        user = UserFactory.create()
        db_request.user = user

        associations = mocker.sentinel.associations
        user_service = mocker.Mock(
            get_account_associations=mocker.Mock(return_value=associations)
        )
        db_request.find_service = lambda iface, **kw: {IUserService: user_service}[
            iface
        ]

        response = views.manage_account_connected_accounts(db_request)

        assert response == {"account_associations": associations}
        assert user_service.get_account_associations.call_args_list == [
            mocker.call(user.id)
        ]
