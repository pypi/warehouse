...
class TestReleaseYankedOrUnyankedEmails:
    @pytest.mark.parametrize(
        "fn, template_name",
        [
            (email.send_release_yanked_email, "release-yanked"),
            (email.send_release_unyanked_email, "release-unyanked"),
        ],
    )
    def test_yanked_or_unyanked_release_emails(
        self, pyramid_request, pyramid_config, monkeypatch, fn, template_name
    ):
        stub_user = pretend.stub(
            id="id",
        ...
        subject_renderer.assert_()
        body_renderer.assert_(username=stub_user.username, project_name=project_name, version=release_version)
        html_renderer.assert_(username=stub_user.username, project_name=project_name, version=release_version)
        assert pyramid_request.task.calls == [pretend.call(send_email)]
        assert send_email.delay.calls == [
            pretend.call(
                f"{stub_user.username} <{stub_user.email}>",
