...
class TestBuildkitePublisherForm:
    def test_validate_and_normalize(self):
        ...
        assert form.normalized_build_tag is None
        assert form.normalized_step_key == "publish"

    @pytest.mark.parametrize("value", [None, "", "  "])
    def test_unset_constraints_normalize_to_none(self, value):
        data = {"organization_slug": "acme", "pipeline_slug": "widgets"}
        if value is not None:
            data.update(build_branch=value, build_tag=value, step_key=value)
        form = buildkite.BuildkitePublisherForm(MultiDict(data))

        assert form.validate(), form.errors
        assert form.normalized_build_branch is None
        assert form.normalized_build_tag is None
        assert form.normalized_step_key is None
