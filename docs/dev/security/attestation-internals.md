...
PyPI's implementation of [PEP 740](https://peps.python.org/pep-0740/).

!!! important
    If you're a user of PyPI, you probably want the [attestation user docs](https://docs.pypi.org/attestations/)
    instead.
...
A [PyPI Publish Attestation](https://docs.pypi.org/attestations/publish/v1) is intended to
...
[SLSA Provenance](https://slsa.dev/spec/v1.0/provenance) is intended to more generally trace a software artifact back
to its source.
...
This section is intended as a high-level walkthrough of a [PEP 740](https://peps.python.org/pep-0740/)
attestation object.
...
* The `signature`, which contains the base64-encoded signature over
  ...
  `statement`, the payload is computed using the [DSSE PAE encoding](https://github.com/secure-systems-lab/dsse/blob/v1.0.0/protocol.md):
...
Attestation object verification is described at a high level in [PEP 740](https://peps.python.org/pep-0740/).
...
!!! warning
    ...
    either [pypi-attestations](https://github.com/pypi/pypi-attestations) or [sigstore-python](https://github.com/sigstore/sigstore-python)'s pre-existing APIs
...
3. Verify that the attestation's `envelope.signature` is valid for
   `envelope.statement`, using the [DSSE PAE encoding](https://github.com/secure-systems-lab/dsse/blob/v1.0.0/protocol.md) and the public key of
