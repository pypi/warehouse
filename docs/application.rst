...
PyPI.org). It does not use any containers, although we may change that
in the future. In the development environment, we use several
`Docker`_ containers, and use `Docker Compose
...
the database. We also use `cabotage
<https://github.com/cabotage/cabotage-app>`__ to manage software and
