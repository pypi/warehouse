...
You can search for packages directly on the PyPI website. The search results can be filtered by [Trove Classifiers](https://pypi.org/classifiers/) — standardized labels that package authors apply to their projects to categorize things like intended audience, license, and supported Python versions. You can also filter by License and Python version (listed under "Programming Language").
