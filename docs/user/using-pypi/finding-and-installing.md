...
You can search for packages directly on the PyPI website. The search results can be filtered by the [Trove Classifiers](https://pypi.org/classifiers/) that package authors have applied to their projects. You can also filter by License and Python version (listed under "Programming Language").
