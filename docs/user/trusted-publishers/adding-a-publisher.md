...
=== "Buildkite"
    ...
    You do not need to look up or enter immutable IDs. Every token must contain
    `organization_id`, `pipeline_id`, and `jti`. PyPI automatically pins the
    publisher to both immutable IDs on the first successful token exchange;
    all future tokens must contain the same IDs. Buildkite includes these
    claims for the `pypi` and `testpypi` audiences. Its OIDC issuer is
