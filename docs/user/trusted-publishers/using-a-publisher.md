...
=== "Buildkite"
    ...
    Registration requires the organization slug and pipeline slug, not the
    immutable IDs. Every token must contain `organization_id`, `pipeline_id`,
    and `jti`; Buildkite includes these for the `pypi` and `testpypi` audiences.
    PyPI pins both IDs on the first successful exchange and requires the same
    values in all later tokens. Any configured build branch, build tag, or
    step key restrictions must also match the publishing job's claims.
