...
* [**Malware reporting**](https://blog.pypi.org/posts/2024-03-06-malware-reporting-evolved/)**:** PyPI relies on its users and the wider security community to help identify and report malicious packages. Users can report malware:  
  * **Via PyPI.org:** Suspicious packages should be reported using the "Report project as malware" link on a project's page  
...
* **Disabling new registrations:** During a large-scale spam or malware attack, PyPI administrators can temporarily disable new user or new project registrations to contain the threat. The PyPI [status page](https://status.python.org/) provides updates during such an event.  
