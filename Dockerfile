...
RUN set -x \
    ...
    && apt-get install --no-install-recommends -y \
        ...
        $(if [ "$DEVEL" = "yes" ]; then echo 'bash curl libjpeg62 postgresql-client build-essential libffi-dev libxml2-dev libxslt-dev libpq-dev libcurl4-openssl-dev libssl-dev'; fi) \
    && apt-get clean \
