...
class MetadataRepository:
        ...
        for delegatee, signers, expiration in delegatees:
            ...
            for signer in signers:
                delegator_metadata.signed.add_key(
                    delegatee.name, Key.from_securesystemslib_key(signer.key_dict)
                )
    ...
    def initialize(
        ...
        for role in TOP_LEVEL_ROLE_NAMES:
            if self.settings[f"tuf.{role}.threshold"] > len(role_signers[role]):
                raise ValueError(
    ...
    def delegate_targets_roles(
        ...
        """
        Performs targets delegation for delegator-to-delegatees items in passed payload.
        ...
        Args:
            payload: Dictionary of delegator as Tuple containing ``DelegatedRole``,
                list of signers and ``datetime`` expiration.
