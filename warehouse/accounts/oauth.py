...
class _NullOAuthClientBase:
    ...
    Subclasses (NullGitHubOAuthClient, NullGitLabOAuthClient) implement
    IOAuthProviderService for use in development.
    """
    ...
    def exchange_code_for_token(self, code: str) -> dict[str, str]:
        ...
        return {
            "access_token": f"mock_{self._provider}_access_token_"
...
class NullGitHubOAuthClient(_NullOAuthClientBase):
    """Null GitHub OAuth client for development. DO NOT use in production."""
...
class NullGitLabOAuthClient(_NullOAuthClientBase):
    """Null GitLab OAuth client for development. DO NOT use in production."""
    ...
    def exchange_code_for_token(self, code: str) -> dict[str, str]:
        """Return mock access token with GitLab-specific fields."""
        return {
            "access_token": f"mock_{self._provider}_access_token_"
            + secrets.token_hex(16),
            "token_type": "bearer",
    ...
    def get_user_info(self, access_token: str) -> dict[str, str | int]:
        ...
        return {
            "id": mock_id,
            "username": f"gitlab_mockuser_{mock_id}",
            "name": f"Mock GitLab User {mock_id}",
            "public_email": f"gitlab_mock_{mock_id}@example.com",
            "state": "active",
            "web_url": f"https://gitlab.com/gitlab_mockuser_{mock_id}",
        }
