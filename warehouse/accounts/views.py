...
def user_profile(username):
    user = current_app.db.accounts.get_user(username)
    ...
    return render_template(
        ...
        user=user,
        projects=current_app.db.packaging.get_projects_for_user(
            user["username"],
        ),
    )
...
def login():
    form = LoginForm(
        request.form,
        authenticator=current_app.db.accounts.user_authenticate,
        translations=current_app.translations,
    )
    ...
    if request.method == "POST" and form.validate():
        ...
        user_id = current_app.db.accounts.get_user_id(form.username.data)
        ...
        # log in process.
        resp = redirect_next(request, default=url_for("warehouse.views.index"))
