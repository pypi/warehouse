...
export default class extends Controller {
  ...
  load() {
    const get_project_objects_count = (project_api_url, project_object, fetchParams) => {
      return fetch(
        `${project_api_url}/${project_object}?` +
          new URLSearchParams({
            state: "opened",
            per_page: 1,
          }),
        ...
        .then((response) => {
          if (response.ok) {
            return response
              .json()
              .then((data) =>
                Object.keys(data).length ? response.headers.get("X-Total-Pages") : 0,
              );
          }
        ...
        .catch((error) => {
          console.error(
            `An error ocured while fetching Gitlab ${project_object} data: ${error.message || error}`,
          );
          return 0;
        });
    };
    ...
    const project_info = fetch(project_api_url, fetchParams).then((response) =>
      response.ok === true ? response.json() : null,
    );

    const issues = get_project_objects_count(project_api_url, "issues", fetchParams);
    const merge_requests = get_project_objects_count(
      project_api_url,
      "merge_requests",
      fetchParams,
    );

    Promise.all([project_info, issues, merge_requests])
      .then(([project, issues_count, merge_requests_count]) => {
        const stats = {
          issues: issues_count,
        ...
        this.gitlabRepoInfoOutlets.forEach((outlet) => outlet.updateStats(stats));
      })
