...
// utils for choosing a package manager
docReady(() => {
  document
    .querySelectorAll(".package-manager-settings-toggle").forEach(function(element) {
      element.onclick = () => {
        const packageManagerSettings = document.querySelector("form.package-manager-settings");
        packageManagerSettings.classList.toggle("hidden");
      };
    });

  document
    .querySelectorAll(".package-manager-settings input")
    .forEach(function (element) {
      element.onchange = () => {
        ...
        const target = document.querySelector("span[data-package-name]");
        const packageName = target.dataset.packageName;
        const projectVersion = target.dataset.projectVersion;

        switch (element.value) {
        case "pip":
          target.innerHTML = `pip install ${packageName}${projectVersion}`;
          break;
        case "poetry":
          target.innerHTML = `poetry add ${packageName}${projectVersion}`;
          break;
        case "pipenv":
          target.innerHTML = `pipenv install ${packageName}${projectVersion}`;
          break;
        case "pdm":
          target.innerHTML = `pdm add ${packageName}${projectVersion}`;
          break;
        }
      };
    });
