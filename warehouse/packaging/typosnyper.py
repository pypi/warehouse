...
def _repeated_characters(project_name: str) -> str | None:
    """
...
def _omitted_chars(project_name: str) -> str | None:
    """
    ...
    """
    # TODO: What's better? A static set here, or a dynamic one with regex expansion?
    #  Or the functionally equivalent `string.ascii_lowercase + string.digits + '-._'`?
...
def _swapped_characters(project_name: str) -> str | None:
    """
...
def _swapped_words(project_name: str) -> str | None:
    """
...
def _common_typos(project_name: str) -> str | None:
    """
        ...
        if character in _TYPO_MAP:
            ...
            for t in _TYPO_MAP[character]:
                ...
                if typo_project_name in _TOP_PROJECT_NAMES:
                    return typo_project_name

    return None


def typo_check_name(project_name: str) -> str | None:
    """
...
# Simple tests to check if the module works Don't ship this to production as is.
if __name__ == "__main__":  # pragma: no cover
    # if passed an argument, check that
    import sys

    # if the first arg is `bench` then time the checks, and use the second arg as input
    if len(sys.argv) > 2:
        if sys.argv[1] == "bench" and len(sys.argv) == 3:
            import time
            import timeit
            ...
            # time each check for the same input
            print("Timing each check for the same input", input_name)
            ...
            for name, func in zip(function_names, functions):
                time_taken = timeit.timeit(partial(func, input_name), number=1000)
                results[name] = time_taken

            for name, time_taken in results.items():
                print(f"{name:<25} {time_taken:.6f} seconds")

            end_time = time.perf_counter()
            total_elapsed = end_time - start_time
            print(f"Total bench duration: {total_elapsed:.6f} seconds")
            exit(0)

    if len(sys.argv) > 1 and not sys.argv[1].startswith("bench"):
        if typo := typo_check_name(sys.argv[1]):
            print(f"Typo detected with: {typo}")
            exit(1)
        else:
            print("No typos detected.")
            exit(0)

    # Should be replaced with proper unit tests
    assert typo_check_name("numpy") is None
    assert typo_check_name("NuMpy") is None
    assert typo_check_name("nuumpy") == "numpy"
    assert typo_check_name("reequests") == "requests"
    assert typo_check_name("sphnx") == "sphinx"
    assert typo_check_name("python-dteutil") == "python-dateutil"
    assert typo_check_name("pythondateutil") == "python-dateutil"
    assert typo_check_name("jinj2a") == "jinja2"
    assert typo_check_name("dateutil-python") == "python-dateutil"
    assert typo_check_name("sphinx-theme-rtd") == "sphinx-rtd-theme"
    assert typo_check_name("requestz") == "requests"
