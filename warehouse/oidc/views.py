...
@view_config(
    route_name="oidc.buildkite.mint_token",
    require_methods=["POST"],
    renderer="json",
    require_csrf=False,
    has_translations=True,
)
def mint_token_from_buildkite_oidc(request):
    def _invalid(errors):
        request.response.status = 422
        print(f"422: {repr(errors)}")
        return {"message": "Token request failed", "errors": errors}

    if request.flags.disallow_oidc(AdminFlagValue.DISALLOW_BUILDKITE_OIDC):
        print("Buildkite OIDC disallowed")
        return _invalid(
            errors=[
                {
                    "code": "not-enabled",
                    "description": (
                        "Buildkite-based trusted publishing functionality not enabled"
                    ),
                }
            ]
        )
    ...
    oidc_service = request.find_service(IOIDCPublisherService, name="buildkite")
    ...
    serialized, dm = macaroon_service.create_macaroon(
        ...
        additional={
            "oidc": {
                "build_branch": claims.get("build_branch"),
                "build_commit": claims.get("build_commit"),
            },
        },
    )
...
def mint_token_from_github_oidc(request):
    def _invalid(errors):
        request.response.status = 422
        return {"message": "Token request failed", "errors": errors}
