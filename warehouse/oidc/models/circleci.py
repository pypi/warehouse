...
class CircleCIPublisherMixin:
    ...
    # Named with circleci_ prefix to avoid collision with PendingOIDCPublisher.organization_id
    ...
    # TODO: Define the specific claims to verify once we finalize the design
    ...
    def __str__(self) -> str:
        return f"CircleCI project {self.circleci_project_id} in organization {self.circleci_org_id}"

    def exists(self, session: Session) -> bool:
        return session.query(
            exists().where(
                and_(
                    self.__class__.circleci_org_id == self.circleci_org_id,
                    self.__class__.circleci_project_id == self.circleci_project_id,
                )
            )
        ).scalar()
